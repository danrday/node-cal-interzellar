...
const { assert: { isFunction, deepEqual, oneOf, isObject } } = require('chai')

const { returnDateArguments, getCurrentDate, allYears, allMonths } = require('../lib/dateArguments')
...
describe('dateArguments', () => {
  ...
  describe('returnDateArguments', () => {
    ...
    it ('when 2 arguments, first 3 letters of first arg should match first 3 letters of a month', () => {
      ...
      const expected = 1
      deepEqual(returnDateArguments(args).month, expected)
    })
